...
<%
	if(us.equals("in")){
		ResultSet rs1=st2.executeQuery("select * from public.documentload where status !='deleted' and author='"+uid+"'");
		String d[]=s.split(" ");
		while(rs1.next()){
...
<%
		...
		ResultSet rs3=st3.executeQuery("select * from public.documentshared where status !='deleted' and sharedto='"+uid+"'");
		while(rs3.next()){
			ResultSet rs2=st2.executeQuery("select * from public.documentload where docid='"+rs3.getString("docid")+"'");
			rs2.next();	
